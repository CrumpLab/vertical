...
An important component of computational reproducibility is specifying the (software) environment in which the computations were done, or should be done. R packages are a robust solution to this problem, because the package's metadata in the DESCRIPTION file can and should include the R packages and their versions that are required for the project. For example, if the project required using a specific version of R, and a specific version of an R package, this information could be written in the DESCRIPTION file. 
